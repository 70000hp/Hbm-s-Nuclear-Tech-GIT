package com.hbm.world.gen.deposits;
...
/** Class was made by MellowArpeggiation, thrown into NTM by 70000hp**/
public class MapGenSaltLake extends MapGenBase {
	...
	protected void func_151538_a(World world, int offsetX, int offsetZ, int chunkX, int chunkZ, Block[] blocks) {
		//&& world.getWorldChunkManager().getBiomeGenAt(chunkX/16, chunkZ/16).temperature >= 4
		...
		if(rand.nextInt(chancePerChunk) == Math.abs(offsetX) % chancePerChunk && rand.nextInt(chancePerChunk) == Math.abs(offsetZ) % chancePerChunk) {
			...
			if(lakeNoise == null){
				lakeNoise = new NoiseGeneratorPerlin(new Random(world.getSeed() + chunkX), 4);
			}
				...
				for(int bz = 15; bz >= 0; bz--) {
					for(int y = 254; y >= 0; y--) {
						...
						if(blocks[index] != null && (blocks[index].isOpaqueCube() || blocks[index].getMaterial().isLiquid())) {
							...
							double r = Math.sqrt(x * x + z * z) + lakeNoise.func_151601_a((xCoord + bx) * 0.2, (zCoord + bz) * 0.2);
